...
function DiscordClient(options) {
		...
		switch (message.t) {
			...
			case "CHANNEL_CREATE":
				...
				if (message.d.is_private) {
					...
					self.directMessages[channelID] = new DMChannel(message.d);
				} else {
					if (self.channels[channelID]) return;
					self.channels[channelID] = new Channel(self.servers[message.d.guild_id], message.d);
				}
				break;
			case "CHANNEL_UPDATE":
				old = copy(self.channels[message.d.id]);
				Channel.update(message.d);
	...
	function Server(data) {
		...
		data.channels.forEach(function(channel) {
			channel.members = {};
			self.channels[channel.id] = new Channel(server, channel);
		});
		data.members.forEach(function(member) {
			self.users[member.user.id] = new User(member.user);
		...
		data.presences.forEach(function(presence) {
			...
			self.users[id].game = presence.game;
	...
	function Channel(server, data) {
		...
		Object.defineProperty(server.channels, channel.id, {
			get: function() { return self.channels[channel.id]; },
			set: function(v) { self.channels[channel.id] = v },
			enumerable: true
		});
	...
	Channel.update = function(data) {
		if (!self.channels[data.id]) {
			self.channels[data.id] = new Channel(data);
			return;
		}
		for (var key in data) {
			self.channels[data.id][key] = data[key];
		}
		delete(self.channels[data.id].is_private);
	}
	function DMChannel(data) {
		var pmchannel = this;
		for (var key in data) {
			this[key] = data[key];
		}
		uIDToDM[message.d.recipient.id] = channelID;
		delete(self.directMessages[channelID].is_private);
	}
	function User(data) {
		for (var key in data) {
			this[key] = data[key];
	...
	Object.defineProperties(this.permission_values = {}, {
		...
		GENERAL_ADMINISTRATOR:{ get: getPerm(3), set: setPerm(3) },
		GENERAL_MANAGE_ROLES:{ get: getPerm(28), set: setPerm(28) },
