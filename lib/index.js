...
function DiscordClient(options) {
	...
	function Server(data) {
		...
		data.channels.forEach(function(channel) {
			channel.members = {};
			self.channels[channel.id] = new Channel(server.id, channel);
		...
		data.presences.forEach(function(presence) {
			...
			for (var key in presence) {
				self.users[id][key] = presence[key];
			}
		});
	...
	function Channel(serverID, data) {
		this.guild_id = serverID;
		for (var key in data) {
			this[key] = data[key];
		}
	}
	...
	['username', 'discriminator', 'bot', 'avatar', 'status', 'game'].forEach(function(k) {
